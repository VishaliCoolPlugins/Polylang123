...
if ( ! function_exists( 'icl_get_languages' ) ) {
	 ...
	 * translated_name  => empty, does not exist in Linguator
	...
	function icl_get_languages( $args = '' ) {
		...
		$languages = LMAT()->model->get_languages_list( array( 'hide_empty' => $args['skip_missing'] ) );
		...
		foreach ( $languages as $lang ) {
			...
			if ( LMAT()->links instanceof LMAT_Frontend_Links && ! empty( $GLOBALS['wp_query'] ) ) {
				$url = LMAT()->links->get_translation_url( $lang );
			}
			...
			$arr[ $lang->slug ] = array(
				'id'               => $lang->term_id,
				'active'           => isset( LMAT()->curlang->slug ) && LMAT()->curlang->slug == $lang->slug ? 1 : 0,
				'native_name'      => $lang->name,
				'missing'          => empty( $url ) ? 1 : 0,
				'translated_name'  => '', // Does not exist in Linguator
...
if ( ! function_exists( 'icl_link_to_element' ) ) {
	...
	function icl_link_to_element( $id, $type = 'post', $text = '', $args = array(), $anchor = '', $echo = true, $return_original_if_missing = true ) {
		...
		$lmat_type = ( 'post' == $type || lmat_is_translated_post_type( $type ) ) ? 'post' : ( 'term' == $type || lmat_is_translated_taxonomy( $type ) ? 'term' : false );
		if ( $lmat_type && ( $lang = lmat_current_language() ) && ( $tr_id = LMAT()->model->$lmat_type->get_translation( $id, $lang ) ) && ( 'term' === $lmat_type || LMAT()->model->post->current_user_can_read( $tr_id ) ) ) {
			$id = $tr_id;
...
if ( ! function_exists( 'icl_object_id' ) ) {
	 ...
	 * @param bool        $return_original_if_missing Optional, true if Linguator should return the original id if the translation is missing, defaults to false.
	...
	function icl_object_id( $element_id, $element_type = 'post', $return_original_if_missing = false, $ulanguage_code = null ) {
		...
		if ( empty( $ulanguage_code ) ) {
			$ulanguage_code = lmat_current_language();
		}
		...
		if ( 'nav_menu' === $element_type ) {
			...
			if ( isset( LMAT()->options['nav_menus'][ $theme ] ) ) {
				foreach ( LMAT()->options['nav_menus'][ $theme ] as $menu ) {
		...
		} elseif ( lmat_is_translated_post_type( $element_type ) ) {
...
if ( ! function_exists( 'wpml_get_language_information' ) ) {
	...
	function wpml_get_language_information( $empty = null, $post_id = null ) {
		...
		return false === $lang ? array() : array(
			...
			'display_name'       => $lang->name, // Seems to be the post language name displayed in the current language, not a feature in Linguator
			'native_name'        => $lang->name,
			'different_language' => lmat_current_language() !== $lang->slug,
		);
	}
}
...
if ( ! function_exists( 'icl_register_string' ) ) {
	...
	function icl_register_string( $context, $name, $string ) {
		LMAT_WPML_Compat::instance()->register_string( $context, $name, $string );
	}
}
...
if ( ! function_exists( 'icl_unregister_string' ) ) {
	...
	function icl_unregister_string( $context, $name ) {
		LMAT_WPML_Compat::instance()->unregister_string( $context, $name );
	}
}
...
if ( ! function_exists( 'icl_t' ) ) {
	 ...
	 * @param bool|null   $has_translation optional, not supported in Linguator
...
if ( ! function_exists( 'icl_translate' ) ) {
	...
	function icl_translate( $context, $name, $string = '', $bool = false, &$has_translation = null, $lang = null ) {
		...
		if ( empty( $string ) ) {
			$string = LMAT_WPML_Compat::instance()->get_string_by_context_and_name( $context, $name );
		}
		return empty( $lang ) ? lmat__( $string ) : lmat_translate_string( $string, $lang );
	}
}
...
if ( ! function_exists( 'wpml_get_copied_fields_for_post_edit' ) ) {
	...
	function wpml_get_copied_fields_for_post_edit() {
		...
		// Don't know what WPML does but Linguator does copy all public meta keys by default.
		...
		$arr['fields'] = array_unique( apply_filters( 'lmat_copy_post_metas', empty( $keys ) ? array() : $keys, false ) );
		return $arr;
	}
}
